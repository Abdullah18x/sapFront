...
class ViewAssignedAssignment extends Component {
  ...
  getFeedback = async () => {
    try {
      let getFeedback = await student.getfeedback(
  ...
  getComments = async () => {
    try {
      let comments = await student.getComments(
        this.props.location.state.fbId,
        this.state.token
      );
      this.setState({
  ...
  comment = async () => {
    ...
    try {
      let getStudent = await student.getStudent(
        this.state.studentId,
        this.state.token
      );
      console.log(getStudent)
      let userDetails = "Student (".concat(getStudent[0].rollNo.concat(")"));
      alert('here')
      console.log(userDetails)
      if (this.state.comment != "") {
        let addComment = await student.comment(
          this.props.location.state.fbId,
          userDetails,
          this.state.comment,
          this.state.token
