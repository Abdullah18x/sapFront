...
class ViewDataSet extends Component {
  ...
  getAssignment = async () => {
    try {
      ...
      let returnedAssignment = await student.getDataSet(
        datasetId,
        this.state.token
      );
  ...
  openIde = () => {
    const url = `http://localhost:5500/?studentId=${this.state.studentId}&assignmentId=${this.props.location.state.datasetId}&assignmentType=${this.props.location.state.assignmentType}&assignedId=${this.props.location.state.assignedSId}&ide=${this.state.token}`;
    window.open(url, "_blank");
  };

  gettingSolution = async () => {
    try {
      let submittedAssignment = await student.getStudentSubmissionS2(
