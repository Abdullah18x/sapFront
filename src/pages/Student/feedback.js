...
class Feedback extends Component {
  ...
  getFeedbacks = async () => {
    try {
      let feedbacks = await student.getfeedbacks(this.state.token);
  ...
  giveFeedback = async () => {
    try {
      ...
      let userDetails = "Student: ".concat(getStudent[0].rollNo);
      console.log(userDetails);
      if (this.state.title != "" && this.state.details != "") {
        let submitF = await student.giveFeedback(
          this.state.studentId,
          this.state.title,
          userDetails,
          this.state.details,
