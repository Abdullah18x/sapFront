    ...
    class TeacherAddAssignments extends Component{
      ...
      assignRightNow(e){
        if(e.target.checked){
          ...
          document.getElementById('dueTime').removeAttribute('disabled')
          document.getElementById('sectionList').removeAttribute('disabled')
          document.getElementById('saveAssignment').setAttribute('disabled',true)
          document.getElementById('assignAssignment').removeAttribute('disabled')
        }else{
          document.getElementById('dueDate').setAttribute('disabled',true)
          document.getElementById('dueTime').setAttribute('disabled',true)
          document.getElementById('sectionList').setAttribute('disabled',true)
          document.getElementById('saveAssignment').removeAttribute('disabled')
          document.getElementById('assignAssignment').setAttribute('disabled',true)
        }
      }
      ...
      saveAssignment = async () => {
        try {
          ...
          // if(this.state.title==='' || this.state.resourceMaterial==='' || this.state.details==='' || this.state.resourceLinks==='' || this.state.solution===''){
          ...
          let uploaded = await lecturer.saveAssignment(data,this.state.lecturerId,this.state.token)
        } catch (error) {
          console.log(error)
        }
      }
                              ...
                              <form>
                                ...
                                <div className="form-row">
                                  <div className="form-group col-md-4">
                                    ...
                                    <input type="date" id="dueDate" name="dueDate" onChange={(e)=>{this.setDueDate(e)}} disabled />
                                    <input type="time" id="dueTime" name="dueTime" onChange={(e)=>{this.setDueTime(e)}} disabled />
                                  </div>
                                  <div class="form-group col-md-3">
                                    <label >Section</label>
                                    <select id="sectionList" class="form-control" onChange={(e)=>{this.setSection(e)}} disabled>
                                ...
                                <div className="form-row">
                                  <div className="form-group col-md-12">
                                    <label htmlFor="inputEmail4">Solution</label>
                                    <textarea className="form-control" id="input6" placeholder="Result" onChange={(e)=>{this.setSolution(e)}}></textarea>
                                  </div>
                                </div>
                                {/* <div className="form-group">
                                ...
                                <div className="form-row">
                                  <button id="assignAssignment" type="button" className="btn btn-primary" onClick={this.assignAssignment} style={{marginRight:'10px'}} disabled>Assign</button>
